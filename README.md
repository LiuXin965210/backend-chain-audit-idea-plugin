...
- **贴近开发流程**：直接从光标方法启动，结果支持筛选、排序、源码跳转以及 Markdown/CSV/Excel/Mermaid 导出。
- **批量接口统计**：支持一次输入最多 100 个 HTTP 接口路径，提交前校验格式并自动去重，按接口维度逐个扫描后导出 Markdown、CSV 或 Excel 批量统计报告。
- **跨工程 Feign 续链**：单接口或单方法审计时，可按显式服务映射把 Feign 调用续链到另一个已打开 IDEA 工程，并继续梳理目标接口资源；同一次扫描内会按 Feign 服务、HTTP 方法和内部路径缓存已处理过的外部接口，避免跨工程来回调用时反复定位同一接口；批量统计不启用该能力。
- **可配置扩展**：可配置排除包、HTTP 工具类、最大深度、MQ 递进和资源去重，不要求所有工程采用相同封装；外围接口中的 `${...}` 占位符会优先从当前工程 `.properties`、`.yml`、`.yaml` 配置文件解析为真实展示值。
- **可取消扫描**：后台任务关闭或取消时会主动取消内部 PSI ReadAction，并在重型索引遍历中检查取消信号；IDE 非用户触发的读动作取消会自动重试，降低 `ProcessCanceledException` 中断扫描的概率；批量统计单接口默认不限制超时时间，也可在设置中配置超时秒数，超时后会标记失败并继续后续接口。
- **MQ 递进保护**：Kafka/RabbitMQ/RocketMQ 的配置占位符 topic 只做完整 key 匹配，避免所有 `*.topic` 配置被误认为同一个消费者链路。
- **可复制证据详情**：资源表证据列支持双击或右键打开可选择文本的详情弹窗，便于选中和复制长链路证据。
...
3. 选择导出格式：默认导出 Markdown，可额外勾选 CSV 或 Excel；至少需要选择一种格式。
...
- 单个接口默认不限制超时时间；如在设置中配置了批量单接口超时秒数，超过后会标记失败并继续扫描后续接口，避免批量任务被某个超大调用链长时间卡住。
- 用户取消后，已完成部分仍会写入导出文件，并在 Markdown 总览中标注取消状态。

批量导出内容：

- **Markdown**：包含总览统计、按 URL 的结果表、成功接口资源汇总、跳过/失败原因明细；MySQL 表在资源汇总中按写入和只读分组展示。
- **CSV**：包含明细行和资源证据列，列包括序号、输入 URL、状态、跳过/失败原因、HTTP 方法、入口、分析方法、方法数、资源数、资源类型、资源名称、操作、置信度、证据。
- **Excel**：包含总览、接口结果、资源汇总三个 sheet；接口结果和资源汇总均按审计接口行维度展示，资源汇总格式与 Markdown 保持一致，MySQL、Kafka、RabbitMQ、RocketMQ 和外围接口均在单元格内按资源换行展示。
- 成功但无资源的接口仍输出入口摘要。
- 跳过和失败的行会明确写出原因，资源列留空。

## 配置

- **使用应用全局配置**：默认开启；开启后配置写入 IDEA 应用级配置，所有工程共享同一份规则。关闭后仅保存到当前工程，适合少数工程单独覆盖规则。
...
- **HTTP 工具类前缀**：配置工程自定义 HTTP wrapper，例如 `jsh.mgt.lib.http.BasicHttpUtil`；调用 URL 支持从局部变量初始化表达式回溯，识别 `@Value` 字段、字符串拼接和 `String.format("%s/...", ...)`，并从当前工程 `.properties`、`.yml`、`.yaml` 中解析 `${...}` 配置占位符。
- **沿 Feign 调用续链到已打开工程**：单接口或单方法审计可开启；每行配置一个 `Feign服务名或占位符=已打开工程名或工程basePath`，例如 `${ylh.cloud.service.stock}=ylh-cloud-service-stock`。
- **最大跨工程 Feign 深度**：限制跨工程递归层数，防止两个工程通过 Feign 来回调用导致循环展开；达到上限时会保留外围接口并输出提示。
- **外围接口查询分组路径**：匹配的外围接口会标记为查询并继续展示、统计、导出和跨工程 Feign 续链；默认按 `/get-`、`/list-`、`/search-` 分组，也可按行增删路径片段，或用 `regex:` 前缀配置正则。
- **批量单接口超时秒数**：默认 `0`，表示不限制超时时间；设置为大于 `0` 时，批量统计中单个接口超过该秒数会标记失败并继续后续接口。
...
- **Feign 服务名**：外围接口资源同时展示 `@FeignClient` 或自定义 `*FeignClient` 显式声明的服务名/URL、HTTP 方法和完整路径；如果服务名或 URL 来自配置占位符，会展示可解析到的真实配置值。
...
- Redis、Spring Cache、标准 Rabbit/Kafka/RocketMQ、阿里云 ONS、`JshRocketMqProducer` / `JshRocketMqListener` 包装器、自定义 MQ 注解、Feign 和可配置 HTTP 工具类，HTTP URL 支持常量、`@Value` 字段、局部变量、字符串拼接、`String.format` 和配置文件占位符解析。
- 资源筛选、排序、全类型去重、源码跳转，以及 Markdown 总览、CSV 明细、Excel 和 Mermaid 导出。
- 批量 HTTP 接口统计，支持 Markdown/CSV/Excel 导出、严格接口定位、interface 唯一实现分析、跳过原因标注和可取消后台进度。
