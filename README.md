...
- **批量接口统计**：支持一次输入最多 100 个 HTTP 接口路径，按接口维度逐个扫描，并导出批量统计报告。
...
- **Kafka/RabbitMQ/RocketMQ**：识别 Template、Producer wrapper、Listener、自定义 MQ 注解、阿里云 ONS API 以及 `@Value` topic/queue 字段。
...
如需在打开 IDEA Project 后自动扫描并导出结果，可在项目根目录创建 `.backend-chain-audit/silent-task.json`。插件会在项目启动后后台检查一次该文件；文件不存在时不会执行任何动作，不会弹输入框、打开工具窗口或弹保存框。
...
- 输出路径必须是项目内相对路径；父目录不存在时会自动创建。
- 执行完成后任务文件会重命名为 `silent-task.<yyyyMMdd-HHmmss>.done.json`。
- 任一任务失败时任务文件会重命名为 `silent-task.<yyyyMMdd-HHmmss>.failed.json`，并生成同名 `.log` 记录失败原因。
