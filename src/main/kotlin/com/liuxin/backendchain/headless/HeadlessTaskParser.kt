...
object HeadlessTaskParser {
    fun parse(text: String): HeadlessTaskFile {
        ...
        if (tasks.isEmpty()) error("tasks 不能为空")
        return HeadlessTaskFile(tasks.mapIndexed { index, value -> parseTask(index + 1, value) })
    }

    private fun parseTask(index: Int, value: Any?): HeadlessTask {
        ...
        return when (type) {
            "http" -> HeadlessTask.Http(task.nonBlankString("input", index), outputs)
            "mq" -> HeadlessTask.Mq(task.nonBlankString("input", index), outputs)
            "batchHttp" -> {
                ...
                HeadlessTask.BatchHttp(normalized, outputs)
            }
            else -> error("第 $index 个任务 type 不支持：$type")
        }
    }

    private fun parseOutputs(index: Int, value: Any?): HeadlessTaskOutputs {
        val outputs = value as? Map<*, *> ?: error("第 $index 个任务缺少 outputs object")
        return HeadlessTaskOutputs(
